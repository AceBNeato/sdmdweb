...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        // Equipment Status Trigger (create only if doesn't exist)
        if (!DB::select("SELECT * FROM information_schema.triggers WHERE trigger_schema = DATABASE() AND trigger_name = 'equipment_status_trigger'")) {
            DB::statement("
        ...
        if (!DB::select("SELECT * FROM information_schema.routines WHERE routine_schema = DATABASE() AND routine_name = 'get_user_equipment_history'")) {
            DB::statement("
